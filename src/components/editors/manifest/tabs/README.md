...
- `overview-tab.tsx`: shows only fields that have been edited relative to the initial manifest snapshot. Those fields remain editable from Overview.
- `descriptive-tab.tsx`: edits manifest-level descriptive fields such as `label`, `summary`, `rights`, and `navDate`.
- The other tab files are placeholders or future expansion points.
